...
PRICE_LIST = {
    ...
    # 🛏️ Домашній текстиль
    "постіль": 650,
    "постіль з прасуванням": 850,
    "рушники": 120,
    "халат": 350,
    "плед": 700,
    "покривало": 800,
    "ковдра синтетична": 900,
    "ковдра пухова": 1200,
    "подушка": 500,
    "наматрацник": 800,
    # 🧵 Ательє
    ...
    "заміна блискавки штани": 350,
    "заміна блискавки куртка": 700,
    "ремонт шва": 250,
    "ремонт кишень": 300,
    "дрібний ремонт": 200,
}
ITEM_ALIASES = {
    ...
    "заміна блискавки штани": [
        "заміна блискавки у штанах",
        "заменить молнию в брюках",
        "замена молнии в штанах",
    ],
    "заміна блискавки куртка": [
        "заміна блискавки у куртці",
        "заменить молнию в куртке",
...
price_menu = ReplyKeyboardMarkup(
    keyboard=[
        [KeyboardButton(text="🧺 Прання"), KeyboardButton(text="💧 Аквачистка")],
        [
            KeyboardButton(text="🧥 Верхній одяг"),
            KeyboardButton(text="🛏️ Домашній текстиль"),
        ],
        [KeyboardButton(text="🧵 Ательє"), KeyboardButton(text="🏢 B2B / по кг")],
        [KeyboardButton(text="⚠️ Після огляду")],
        [KeyboardButton(text="🔙 Назад до меню")],
    ],
    resize_keyboard=True,
)
...
async def price_washing(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "👔 Сорочка — від 250 грн\n"
        "👕 Футболка / майка — від 180 грн\n"
        "👖 Штани — від 250 грн\n"
        "👖 Джинси — від 280 грн\n"
        "🩳 Шорти — від 180 грн\n"
        "🧥 Худі / світшот — від 350 грн\n"
        "🧶 Светр / кофта — від 350 грн\n\n"
        f"{PRICE_NOTE}"
...
@dp.message(F.text == "💧 Аквачистка", any_state)
async def price_aqua_cleaning(message: Message, state: FSMContext):
    await state.clear()

    await message.answer(
        "💧 <b>Аквачистка</b>\n\n"
        "Делікатний догляд для речей, які потребують м’якої обробки без агресивної хімії.\n\n"
        "👗 Сукня — від 650 грн\n"
        "🧥 Куртка — від 850 грн\n"
        "🧥 Пуховик короткий — від 1200 грн\n"
        "🧥 Пуховик довгий — від 1500 грн\n"
        "🧥 Пальто текстильне — від 1300 грн\n"
        "🧥 Жилет — від 750 грн\n"
        "🧥 Плащ / тренч — від 1000 грн\n\n"
        "📌 Аквачистка підходить для речей, де потрібен більш делікатний догляд.\n"
        "Остаточна вартість залежить від матеріалу, наповнювача, довжини виробу та ступеня забруднення.\n\n"
        "Щоб оформити замовлення, натисніть <b>«Здати речі»</b>.",
        reply_markup=price_menu,
    )
...
async def price_home_textile(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "Постільна білизна комплект — від 650 грн\n"
        "Постільна білизна з прасуванням — від 850 грн\n"
        "Рушники — від 120 грн / кг\n"
        "Халат — від 350 грн\n"
        "Плед — від 700 грн\n"
        "Покривало — від 800 грн\n"
        "Ковдра синтетична — від 900 грн\n"
        "Ковдра пухова — від 1200 грн\n"
        "Подушка — від 500 грн\n"
        "Наматрацник — від 800 грн\n"
        "Штори / тюль — після огляду\n\n"
        f"{PRICE_NOTE}"
        "Щоб оформити замовлення, натисніть <b>«Здати речі»</b>.",
        reply_markup=price_menu,
    )
...
async def price_atelier(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "Заміна блискавки у штанах / джинсах — від 350 грн\n"
        "Заміна блискавки у куртці — від 700 грн\n"
